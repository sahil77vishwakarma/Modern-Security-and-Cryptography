...
#Alice sendting the encrypted message
...
#Eve's attack on the double DES
lookup = {}
for i in range(256):
...
for i in range(256):
    ...
    if k.decrypt(cipher) in lookup:
        print("key_11:", i )
        print("key_21:", lookup[k.decrypt(cipher)])
        key_1 = bytes([i, 0, 0, 0, 0, 0, 0, 0])
        key_2 = bytes([lookup[k.decrypt(cipher)], 0, 0, 0, 0, 0, 0, 0])
        k1 = des(key_1, ECB, iv, pad=None, padmode=PAD_PKCS5)
        k2 = des(key_2, ECB, iv, pad=None, padmode=PAD_PKCS5)
        print("Eve break the double DES:", k2.decrypt(k1.decrypt(cipher)))
        break
